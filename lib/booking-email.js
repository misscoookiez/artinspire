...
export async function sendCompletedClassPaymentConfirmation({ email, name, purchase, amountCents }) {
  const inbox = process.env.STUDIO_INBOX_EMAIL || "misscoookiez@gmail.com";
  const title = purchase === "trial" ? "Trial class" : purchase === "group" ? "Group class" : "Other studio payment";
  ...
  await sendEmail({
    ...
    subject: `Class payment received — ${title}`,
    html: emailShell({ eyebrow: "CLASS PAYMENT RECEIVED", title: "A class payment has arrived.", intro: `${safeName} paid for a completed ${title.toLowerCase()}.`, content: `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse">${detailRow("Class", title)}${detailRow("Amount", amount)}${detailRow("Name", safeName)}${detailRow("Email", `<a href="mailto:${safeEmail}" style="color:#1f7d89">${safeEmail}</a>`)}</table>`, footer: false }),
    text: `Class payment received\nClass: ${title}\nAmount: ${amount}\nName: ${name || "Not provided"}\nEmail: ${email || "Not provided"}`,
  });
  return sendEmail({
    to: email,
    subject: "Your Art Studio Inspire payment is confirmed",
    html: emailShell({ eyebrow: "PAYMENT CONFIRMED", title: "Thank you.", intro: `We received your payment for a ${title.toLowerCase()}.`, content: `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse">${detailRow("Class", title)}${detailRow("Amount paid", amount)}</table><p style="margin:18px 0 0;color:#4f6271;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6">We are glad you came to paint with us.</p>` }),
    text: `Thank you. We received your ${amount} payment for a ${title.toLowerCase()}.`,
  });
}
