...
export async function sendGroupApplication({name,email,group}){
  ...
  const confirmation=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${process.env.RESEND_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({from:process.env.RESEND_FROM_EMAIL,to:[email],subject:"We received your Art Studio Inspire application",html:`<p>Hi ${safeName||"there"},</p><p>Thank you for applying for <strong>${safeGroup}</strong>. We will get back to you with the best first date and next step.</p><p>— Art Studio Inspire</p>`})});
