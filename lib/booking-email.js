...
const eventAnnouncementCopy = {
  en: { subject:"A new event at Art Studio Inspire", eyebrow:"NEW EVENT", title:"Something new is taking shape.", intro:"A new studio event has just been added.", button:"Explore the event" },
  lv: { subject:"Jauns pasākums Art Studio Inspire", eyebrow:"JAUNS PASĀKUMS", title:"Top kaut kas jauns.", intro:"Studijas kalendārā tikko pievienots jauns pasākums.", button:"Apskatīt pasākumu" },
  ru: { subject:"Новое событие в Art Studio Inspire", eyebrow:"НОВОЕ СОБЫТИЕ", title:"Появляется что-то новое.", intro:"В календарь студии только что добавлено новое событие.", button:"Посмотреть событие" },
};
...
export async function sendEventAnnouncement({ email, event, locale = "en" }) {
  const actualLocale = emailLocale(locale); const copy = eventAnnouncementCopy[actualLocale];
  const title = event[`title_${actualLocale}`] || event.title_en; const summary = event[`summary_${actualLocale}`] || event.summary_en || "";
  const when = bookingDateTime(event.starts_at, event.ends_at, actualLocale);
  return sendEmail({ to:email, subject:`${copy.subject} — ${title}`, html:emailShell({ eyebrow:copy.eyebrow, title:copy.title, intro:copy.intro, content:`<p style="margin:0 0 8px;color:#172739;font-family:Arial,Helvetica,sans-serif;font-size:23px;line-height:1.2">${escapeHtml(title)}</p><p style="margin:0 0 18px;color:#4f6271;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6">${escapeHtml(when)}<br>${escapeHtml(summary)}</p>${button(`${SITE_URL}/events`,copy.button)}` }), text:`${copy.title}\n\n${title}\n${when}\n${summary}\n\n${SITE_URL}/events` });
}
