...
export function bookingCalendar({title,startsAt,endsAt,token}){
  ...
  return `BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//Art Studio Inspire//Bookings//EN\r\nBEGIN:VEVENT\r\nUID:${token}@artstudioinspire\r\nDTSTAMP:${stamp(Date.now())}\r\nDTSTART:${stamp(startsAt)}\r\nDTEND:${stamp(endsAt)}\r\nSUMMARY:${escapeIcs(title)}\r\nDESCRIPTION:${escapeIcs(`Art Studio Inspire · Miera iela 17, Riga. Entrance is beside M50; go down the stairs to the studio door. Door code: 2043a. Manage this booking: ${url}`)}\r\nLOCATION:Miera iela 17, Riga\r\nURL:${url}\r\nEND:VEVENT\r\nEND:VCALENDAR\r\n`;
}
...
export async function sendBookingConfirmation({email,name,title,startsAt,endsAt,token}){
  ...
  const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${process.env.RESEND_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({from:process.env.RESEND_FROM_EMAIL,to:[email],subject:`See you soon — ${title}`,html:`<div style="max-width:600px;margin:0 auto;font-family:Arial,Helvetica,sans-serif;color:#17223a;line-height:1.55"><p>Hi ${safeName},</p><p><strong>We look forward to welcoming you at Art Studio Inspire.</strong></p><p><strong>Your booking:</strong> ${safeTitle}</p><p><a href="${calendarUrl}">Add this booking to your calendar</a></p><div style="margin:24px 0;padding:18px 20px;border-left:3px solid #62ddea;background:#f2fbff"><strong>Finding the studio</strong><br>Art Studio Inspire · Miera iela 17, Rīga<br>Entrance is beside the M50 shop. Go down the stairs to the studio door.<br><strong>Door code: 2043a</strong></div><p>Questions before you come? Reach out any way that is easiest:</p><ul><li><a href="tel:+37128809550">Call +371 2880 9550</a></li><li><a href="https://wa.me/37128809550">Message us on WhatsApp</a></li><li><a href="https://www.instagram.com/artstudio.inspire/">Send an Instagram DM</a></li></ul><p><small>You can cancel online and receive an automatic refund until 24 hours before the start time. <a href="${manageUrl}">Manage this booking</a>.</small></p><p>— Art Studio Inspire</p></div>`})});
...
export async function sendInquiry({name,email,topic,message}){
  const inbox=process.env.STUDIO_INBOX_EMAIL || "misscoookiez@gmail.com";
  const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${process.env.RESEND_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({from:process.env.RESEND_FROM_EMAIL,to:[inbox],reply_to:email,subject:`Studio inquiry — ${escapeHtml(topic)}`,html:`<p><strong>New Art Studio Inspire inquiry</strong></p><p><strong>Topic:</strong> ${escapeHtml(topic)}<br><strong>Name:</strong> ${escapeHtml(name)}<br><strong>Email:</strong> ${escapeHtml(email)}</p><p><strong>Message:</strong></p><p>${escapeHtml(message).replace(/\n/g,"<br>")}</p>`})});
  if(!response.ok) throw new Error("We could not send your message just now.");
}

export async function sendEventInquiryConfirmation({ name, email }) {
  const safeName = escapeHtml(name || "there");
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [email],
      subject: "We received your Art Studio Inspire event request",
      html: `<div style="max-width:600px;margin:0 auto;font-family:Arial,Helvetica,sans-serif;color:#17223a;line-height:1.55"><p>Hi ${safeName},</p><p><strong>Thank you — we received your event request.</strong></p><p>We will be in touch soon to confirm the details, availability and final price.</p><p>In the meantime, you can reach us at Art Studio Inspire:</p><ul><li><a href="tel:+37128809550">+371 2880 9550</a></li><li><a href="https://wa.me/37128809550">WhatsApp</a></li><li><a href="https://www.instagram.com/artstudio.inspire/">Instagram</a></li></ul><p>Art Studio Inspire<br>Miera iela 17, Rīga</p></div>`,
    }),
  });
  if (!response.ok) throw new Error("Event-request confirmation could not be sent.");
}
