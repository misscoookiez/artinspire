                    ...
                    {calendarKind === "inquiry" ? null : calendarKind === "event-format" ? (
                      ...
                      <div className="inspire-event-booking-flow">
                        ...
                        <div className="inspire-event-time-summary"><b>{eventOffer.label} · {eventFormat === "custom" ? (lang === "lv" ? "no €15 / h" : lang === "ru" ? "от €15 / ч" : "from €15 / h") : `€${selectedEventEstimate}`}</b><span>{eventDate ? `${eventDate} · ${String(eventStartHour).padStart(2, "0")}:00–${String(eventStartHour + eventDuration).padStart(2, "0")}:00` : (lang === "lv" ? "Izvēlies datumu" : lang === "ru" ? "Выберите дату" : "Choose a date")}</span><small>{eventFormat === "custom"
                          ...
                          : eventNeedsAttendees
                            ? (lang === "lv" ? `${eventAttendees} ${eventAttendeeLabel} × €${eventPersonRate} = €${eventClassEstimate}. Gleznošanas laiks: ${eventOffer.duration} h. Papildu studijas laiks: 2 h iekļauts. Papildu laiks: ${selectedEventExtraHours} h × €15 = €${selectedEventExtraPrice}.` : lang === "ru" ? `${eventAttendees} ${eventAttendeeLabel} × €${eventPersonRate} = €${eventClassEstimate}. Творческая часть: ${eventOffer.duration} ч. Ещё 2 ч в студии включены. Дополнительное время: ${selectedEventExtraHours} ч × €15 = €${selectedEventExtraPrice}.` : `${eventAttendees} ${eventAttendeeLabel} × €${eventPersonRate} = €${eventClassEstimate}. Painting activity: ${eventOffer.duration} h. An additional 2 h at the studio are included. Extra time: ${selectedEventExtraHours} h × €15 = €${selectedEventExtraPrice}.`)
                            : (lang === "lv" ? `Lielais kopīgais audekls: €200. Gleznošanas laiks: ${eventOffer.duration} h. Papildu studijas laiks: 2 h iekļauts. Papildu laiks: ${selectedEventExtraHours} h × €15 = €${selectedEventExtraPrice}.` : lang === "ru" ? `Общий большой холст: €200. Творческая часть: ${eventOffer.duration} ч. Ещё 2 ч в студии включены. Дополнительное время: ${selectedEventExtraHours} ч × €15 = €${selectedEventExtraPrice}.` : `Large shared canvas: €200. Painting activity: ${eventOffer.duration} h. An additional 2 h at the studio are included. Extra time: ${selectedEventExtraHours} h × €15 = €${selectedEventExtraPrice}.`)}</small><em>{lang === "lv" ? "Aptuvena cena — gala summu apstiprināsim pēc detaļu saskaņošanas." : lang === "ru" ? "Ориентировочная цена — подтвердим итог после согласования деталей." : "Estimated price — we will confirm the final total once the details are agreed."}</em><strong className="inspire-event-total">{lang === "lv" ? `Kopā: ${selectedEventEstimate}€` : lang === "ru" ? `Итого: ${selectedEventEstimate}€` : `Total: ${selectedEventEstimate}€`}</strong></div>
                      </div>
                    ) : calendarKind === "gift" || calendarKind === "pass" ? (
