...
const groups = [
  ["Ceturtdiena", "16:00–18:00", "JAUNIEŠI · 8–16 G."],
  ["Ceturtdiena", "18:30–20:30", "PIEAUGUŠIE"],
  ["Sestdiena", "11:00–13:00", "JAUNIEŠI · 8–16 G."],
...
const englishGroups = [
  ...
  ["Saturday", "11:00–13:00", "YOUTH · AGES 8–16"],
...
const russianGroups = [
  ["Четверг", "16:00–18:00", "ДЕТИ И ПОДРОСТКИ · 8–16 ЛЕТ"],
  ["Четверг", "18:30–20:30", "ВЗРОСЛЫЕ"],
  ["Суббота", "11:00–13:00", "ДЕТИ И ПОДРОСТКИ · 8–16 ЛЕТ"],
...
export default function InspirePage() {
  ...
  const [formMode, setFormMode] = useState("pay");
  ...
  const t = words[lang],
    activeGroups =
      lang === "lv" ? groups : lang === "ru" ? russianGroups : englishGroups,
    activeProducts =
      lang === "lv"
        ? products
        : lang === "ru"
  ...
  const weeklyColumns = [
    ...
    activeGroups.slice(4, 6),
  ];
  const weeklyGroupKeys = [
    "thu-youth",
    "thu-adult",
    "sat-youth",
    "sat-adult",
  ...
  const openApplication = (label, groupKey) => {
    setBooking({ kind: "weekly", groupKey });
    setSelection("");
    setCalendarKind("all");
    setCheckoutOption("");
    setFormMode("apply");
    ...
    <main className="inspire">
      ...
      <section id="nodarbibas" className="inspire-section">
        ...
        <div className="inspire-weekly-columns">
          {weeklyColumns.map((column, columnIndex) => (
            <div key={column[0][0]}>
              <h3>{column[0][0]}</h3>
              {column.map(([day, time, audience], index) => (
                <button
                  key={`${day}-${time}`}
                  onClick={() =>
                    openApplication(
                      `${day} · ${time} · ${audience}`,
                      weeklyGroupKeys[columnIndex * 2 + index],
                    )
                  }
                >
                  <b>{time}</b>
            ...
            {sent ? (
              <>
                ...
                <p>
                  {formMode === "apply"
                    ? lang === "lv"
                      ? "Vieta regulārajā grupā ir rezervēta. Drīzumā sazināsimies par pirmo datumu un praktiskajām detaļām."
                      : lang === "ru"
                        ? "Место в регулярной группе забронировано. Скоро свяжемся о первой дате и организационных деталях."
                        : "Your place in the weekly group is reserved. We will be in touch about the first date and practical details."
                    : t.demo}
                </p>
                <button
                  type="button"
              ...
              <>
                {formMode === "apply" ? (
                  <>
                    <p className="inspire-kicker">{t.booking}</p>
                    <h2>
                      {lang === "lv"
                        ? "Pieteikums regulārai grupai"
                        : lang === "ru"
                          ? "Заявка в постоянную группу"
                          : "Apply for a weekly group"}
                    </h2>
                    <p>{status}</p>
                  </>
                ) : (
