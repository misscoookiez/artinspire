...
export async function GET() {
  ...
  const withAvailability=(session)=>({...session,remaining:Math.max(0,session.capacity-(booked[session.id]||0)-(held[session.id]||0))});
  ...
  return NextResponse.json({
    ...
    classAvailability:(classesResult.data||[]).map(session=>({id:session.id,remaining:withAvailability(session).remaining})),
    classSessions:(classesResult.data||[]).map(withAvailability),
