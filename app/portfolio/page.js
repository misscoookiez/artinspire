...
const art = [
  ...
  {src:"/art/black-cat.webp",title:"Black cat",category:"DRAWINGS",medium:"Watercolour and ink",year:"STUDIO ARCHIVE"},
  {src:"/art/abstract-bricks.webp",title:"Abstract bricks",category:"ABSTRACT",medium:"Oil and acrylic",year:"STUDIO ARCHIVE"},
  {src:"/art/prism.webp",title:"Prism",category:"ABSTRACT",medium:"Oil on canvas",year:"STUDIO ARCHIVE"},
...
export default function Portfolio(){
  ...
  return <main className="sandra-portfolio">
    ...
    <section className="portfolio-intro"><div><p>WHO LOOKS OUTSIDE, DREAMS;<br/>WHO LOOKS INSIDE, AWAKES.</p><small>— CARL JUNG</small></div><img src="/art/sandra-portrait-hero.webp" alt="Sandra Rudzīte in her studio" decoding="async" fetchPriority="high"/></section>
    <section className="portfolio-title"><p>SELECTED WORKS · 2014—2026</p><h1>{filter}</h1></section>
    {shown.length ? <div className="portfolio-gallery">{shown.map((item,i)=><button className="art-card" onClick={()=>setSelected(item)} key={`${item.title}-${i}`}><figure><img src={item.src} alt={item.title} loading="lazy" decoding="async"/><figcaption>{item.title}<span>VIEW →</span></figcaption></figure></button>)}</div> : <p className="portfolio-empty">New works in this collection will appear here soon.</p>}
