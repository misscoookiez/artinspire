...
export default function BookingManagement({ booking, token, canCancel }) {
  ...
  return <main className="manage-page"><header><a href="/">SANDRA RUDZĪTE</a><a href="/inspire">ART STUDIO INSPIRE</a></header><section><p>BOOKING MANAGEMENT</p><h1>Your studio<br/><em>time.</em></h1><div className="manage-card"><span>{booking.kind === "private" ? "PRIVATE SESSION" : "GROUP CLASS"}</span><h2>{booking.title}</h2><p>{start}</p><small>Status · {booking.status}</small><a href={`/api/bookings/${token}/calendar`}>ADD TO CALENDAR (.ICS) →</a></div>{canCancel?<div className="manage-action"><h2>Need to change plans?</h2><p>Cancel at least 24 hours before the session starts and the payment will be refunded automatically to the original payment method.</p><button onClick={cancel} disabled={Boolean(state)}>{state || "CANCEL & REFUND BOOKING"}</button></div>:<div className="manage-action"><h2>This booking can no longer be cancelled online.</h2><p>The 24-hour cancellation window has passed, or this booking has already been changed. Please contact the studio if you need help.</p><a href="mailto:misscoookiez@gmail.com">CONTACT THE STUDIO →</a></div>}</section></main>;
}
